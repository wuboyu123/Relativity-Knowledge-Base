...
url: https://help.relativity.com/Server2025/Content/Relativity/Analytics/Cluster_visualization.htm
collection: user
fetched_at: 2026-06-22T06:05:33+00:00
sha256: 98d7df1f0cc93b78bb6797d15efc028a5ef2be6929cfd950610543730b791587
---

Cluster Visualization Skip To Main Content Account Settings Logout
...
- Click on the Documents tab to open the Cluster browser.

- Select a cluster on the Cluster browser.

- Click the Visualize Cluster button, or right-click the cluster and click Visualize Cluster .
...
If you select a new first level cluster from the Cluster browser, the widget refreshes and the title of the widget is updated to reflect your new cluster selection. If you select a new subcluster, the widget refreshes to display the relevant heat mapping. With any change in cluster selection in the Cluster browser, any cluster filters that have been created from selecting clusters are discarded from the search panel.
...
You do not have to be on the Clusters browser to add a cluster visualization. You can create a cluster visualization from the Folders and Field Tree browsers as well.
...
To view multiple cluster set visualizations, you must create a different dashboard for each one. There can be only one cluster visualization widget on a Dashboard at one time.
...
- Control+Left-click - Control+left-click to select one or more clusters on the circle pack visualization to apply as a filter. Once you select one or more clusters, you must click Apply to apply your selection as a filter or click Cancel . See Applying filters to visualized clusters .
...
Right-clicking outside a cluster reveals only the Select All and Clear Selection actions.
...
- Control+Left-click - Control+left-click to select one or more clusters on the dial visualization to apply as a filter. Once you select one or more clusters, you must click Apply to apply your selection as a filter or click Cancel . See Applying filters to visualized clusters .
...
Right-clicking outside the dial reveals only the Select All and Clear Selection actions.
...
The Document Breakdown is most helpful when you are navigating outside the Cluster browser on the Documents tab (e.g., folder or field browsers) and viewing documents that are not necessarily within the cluster being visualized in the dashboard widget.
...
- Click one of the following selection options:

- Select - selects the currently selected cluster.

- Select All - selects all clusters.

Right-clicking anywhere inside the circle pack visualization also reveals the select all action.
...
Right-clicking anywhere inside the dial visualization also reveals the select all action
...
Right-clicking anywhere inside of the nearby clusters visualization also reveals the select all action.
...
- Verify the filter is applied on the search panel.
