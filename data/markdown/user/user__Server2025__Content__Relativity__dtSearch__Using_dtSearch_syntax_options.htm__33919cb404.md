---
title: "Using dtSearch syntax options"
url: https://help.relativity.com/Server2025/Content/Relativity/dtSearch/Using_dtSearch_syntax_options.htm
collection: user
fetched_at: 2026-06-22T06:07:41+00:00
sha256: 4612c9f017adaacbc618a2984ab10fae7030e44b20c7a91bf0c460109482299c
---

Using dtSearch syntax options Skip To Main Content Account Settings Logout
...
For details on parsing proximity and Boolean strings in search conditions, see dtSearch - How are Proximity and Boolean (AND/OR) parsed in search conditions? knowledge base article on the Relativity Community site.
...
You can also use NOT in a proximity search as illustrated by the NOT W/N, NOT Within N words, operator. See W/N operator .
...
A keyword search is an SQL full text search, which queries individual fields. Keyword searches do not return the same results as dtSearch when using the NOT operator to query across multiple fields. See NOT operator .
...
By default, dtSearch evaluates OR expressions before AND expressions: A AND (B OR C). Unlike dtSearch, the order of precedence for a keyword search evaluates AND expressions before OR expressions: (A AND B) OR C. See Keyword search .
...
You can make special characters searchable in a dtSearch index. However, you must escape some characters when using regular expressions. For more information, see Searching for symbols .
...
This character slows searches when used near the beginning or middle of a word.
...
You can break up complex expressions with OR connectors into separate searches. Search apple w/10 "orange tree" OR banana w/10 "orange tree" to return the same results as (apple OR banana) W/10 "orange tree" .
...
dtSearch displays a warning message when you enter an ambiguous search request.
...
When connecting proximity expressions using Boolean operators, you must use parentheses.
...
- To include either symbol in the dtSearch index, you must add it to the alphabet file. For more information, see Searching for symbols .
...
- dtSearch indexes are case insensitive by default. All characters in a dtSearch index normalize to lowercase. Relativity does not support case-sensitive dtSearch indexes. For example, if your exact phrase is an acronym like ACT , these will be included in the index: act , AcT , aCt , and similar. See Case-insensitive indexes for more information.
