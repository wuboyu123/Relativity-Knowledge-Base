---
title: "Monitoring an Active Learning project"
url: https://help.relativity.com/Server2025/Content/Relativity/Active_Learning/Monitoring_an_Active_Learning_project.htm
collection: user
fetched_at: 2026-06-22T06:08:04+00:00
sha256: 8950e20c59d96800d9b6c0582351d08bd7fe20b603c8c6112e09a58fc0d91830
---

Monitoring an Active Learning project Skip To Main Content Account Settings Logout
...
The actual Active Learning process takes place using a combination of time and detection of new coding decisions. After the Active Learning model completes its first build (once you have at least five documents coded with the positive choice and five coded with the negative choice), it builds at maximum every 20 minutes after the previous build to include coding decisions not included in the most recent build. If reviewer activity has been idle for five minutes and there are coding decisions not included in the most recent build, the model will start a build.
...
Each time an admin accesses the Project Home page - via a page refresh or from a different page - the latest data will reflect in the Project Home display.
...
If a coding decision is updated on a document reviewed in the queue, it will not change to a manually selected document.
...
When you update the <Positive Choice> Cutoff value, the value is updated in all three places where it’s used in the application: Project Validation, Update Ranks, and Project Settings.
...
- CSR - <Project Name> Cat. Set::Category Name - the positive or negative choice based on the <Positive Choice> Cutoff value. Documents below the cutoff are automatically given the negative choice category.

The values in the rank categorization fields are distinct from the coding decision on the document. For example, a document may be coded not responsive, but if the rank is at or above the <Positive Choice> Cutoff value, it is categorized as responsive.
...
The index you select must be associated with an Active Learning project that has been built (at least five documents coded with the positive designation and five coded with the negative designation).
...
If you see a disproportionately high number of documents ranked at 50, this may indicate that you have a high number of empty documents in your project. For more information, see Troubleshooting a large group of documents ranked at 50 .
...
- Red (Suppressed Duplicates) - documents that are suppressed by the suppress duplicates setting after a textually similar document has been coded.

When a full population is performed, all previously identified suppressed documents are marked as "Not Reviewed" in the Document Rank Distribution chart.
...
Documents that consist only of words that are not present in coded documents will also not be rankable by the model.
...
The AL Designation field could be named Responsive or Relevance but for this example we have created a single choice field named AL Designation.
...
- Default Display Type : Pie Chart
...
- Group by : Categories - Active Learning Project Cat. Set
...
- Group by : CSR - Active Learning Project Cat. Set::Category Rank

- Group By Results Returned : All

- Pivot on : AL Designation

- Pivot on Results Returned : All

- Sort on : CSR - Active Learning Project Cat. Set::Category Rank (Ascending)

- Default Display Type : Stacked Bar Chart
...
- Group by : Active Learning Project Reviewers::User
...
These are documents that Active Learning is predicting as non-responsive, but the reviewer has coded as responsive. While such conflicts usually cannot be eliminated entirely, too many of them could be a sign of reviewer inconsistency, not coding on the "four corners" of the document, or a saved search for the index which returns something other than the extracted text field.

- Select the Non-Responsive items coded by reviewers. Select the Responsive classified items. Consult the Reviewers::User widget to see who coded these documents if they were coded in a queue.

These are documents that Active Learning is predicting as responsive, but the reviewer has coded as non-responsive. While such conflicts usually cannot be eliminated entirely, too many of them could be a sign of reviewer inconsistency, not coding on the "four corners" of the document, or a saved search for the index which returns something other than the extracted text field.
