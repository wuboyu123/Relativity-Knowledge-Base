...
url: https://help.relativity.com/Server2025/Content/Relativity/Processing/Deduplication_considerations.htm
collection: user
fetched_at: 2026-06-22T06:06:01+00:00
sha256: c071d861a8d0c255616751c60c1b32ba6fc59f30ee7918bf79dc20f65725c10a
---

Deduplication considerations Skip To Main Content Account Settings Logout
...
Relativity can't calculate the MD5 hash value if you have FIPS (Federal Information Processing Standards cryptography) enabled for the worker manager server.
...
ICS/VCF files are deduplicated not as emails but as loose files based on the SHA256 hash. Since the system now considers these loose files, Relativity is no longer capturing the email-specific metadata that it used to get as a result of ICS/VCF files going through the system's email handler. For a detailed list of all the metadata values populated for ICS and VCF files, see ICS and VCF file considerations .
...
When using deduplication, the following may apply:

-

Relativity uses the custodian field that you mapped before publishing, and not the All Custodians field. The All Custodians field is typically used for reporting purposes and is not used in deduplication.
