---
title: "Assigning batches and checking batches in and out"
url: https://help.relativity.com/Server2025/Content/Relativity/Batches/Checking_batches_in_and_out.htm
collection: user
fetched_at: 2026-06-22T06:14:31+00:00
sha256: a126099dec71aaa438dd8d4729e8b536e4bb4de0ddeb61e50a02415fed2dc2aa
---
...
- Applying views to the batches tab, creating any list of batches you’d like to see. You can restrict these views.
...
Reviewers can assign batches to other reviewers with the Assign Batches permission. Reviewers without this permission can only check out batches for themselves. By default, reviewers don't have this permission.
...
A user who belongs only to the Workspace Admin Group will not appear in the Assign To drop-down list for batch check-out. To be assignable as a batch reviewer, that user must also belong to at least one other group in the workspace (not the System Administrators or the Workspace Admin group) that has access to the workspace and the permission to view batches. A workspace admin with no additional reviewer group membership cannot be assigned batches.
...
System admins can't check out batches using the Me link.
...
- Select one of the options on the Batch Edit pop-up. Depending on permissions, you might not see the Reassign To button.

- Check in as pending - you can check in the batch as pending for the assigned user.

- Check in as completed - you can check in the batch as completed for the assigned user.

You can select the Check in as completed option even if not all of the documents have been reviewed.

- Reassign - you can assign the batch to another user.
