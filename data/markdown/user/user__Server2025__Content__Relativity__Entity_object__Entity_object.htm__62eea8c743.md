---
title: "Entity object"
url: https://help.relativity.com/Server2025/Content/Relativity/Entity_object/Entity_object.htm
collection: user
fetched_at: 2026-06-22T06:05:45+00:00
sha256: 73efbd49c3b2136d44dee8560ef4240161477a5fcf16682f3a3913e61e8f82c2
---

Entity object Skip To Main Content Account Settings Logout
...
Set this through the Edit Permissions option on the Classification field layout.
...
- Load entities via the Relativity Desktop Client. See Importing entities through the RDC .
...
- Select a layout from the drop-down list. For more information, see the following:

Layouts only appear for the applications you have installed.
...
- Compile or locate the entity load file you wish to import. For more information about the load file, see the following:
...
If the Legal Hold application is installed in the workspace, it will negatively impact the performance of the Mass delete operation.
...
When you upgrade to Server 2025 , existing entities are placed into the Last Name field on the Entity layout. If you need to edit an upgraded entity, you need to parse out the first and last names, as both of those fields require values when you save the entity.
...
You can't delete an entity already associated with a processing data source. For more information, see Adding a data source .
...
- Other - select this if the entity of the data you wish to process isn't an individual but is, for example, just a company name. You can also select this if you wish to enter an individual's full name without having that name include a comma once you export the data associated with it.
...
- Select one of the Case Dynamics entities layouts. See Default Case Dynamics layouts .
...
- Key Organization Extended Layout

- Key Person Basic Layout

- Key Person Extended Layout
...
- Communicator - Analytics - select this option if the entity is impacted by name normalization in Analytics.

- Custodian - Processing - select this option if the entity is impacted by Processing.

- Key - Case Dynamics - select this option to use the entity in Case Dynamics.
